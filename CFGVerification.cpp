...
struct CFGVerification : public ModulePass {
  ...
  private:
      ...
      for (Function &Fn : Md) {
        ...
        if (!Fn.isDeclarationForLinker() && !Fn.hasExternalLinkage() && !Fn.hasNUses(0)) {
          for (User *U : Fn.users()) {
            ...
            BBCalls.insert(std::pair<BasicBlock*, BasicBlock*>(ParentBB, ChildBB));
          }
        }
      }
    }
     ...
     * @param Md
     */
    void initializeEntryFunctions(Module &Md, const std::map<BasicBlock *, int> &BBSigs, const std::map<BasicBlock*, BasicBlock*> &BBCalls, Value* G, Value *D) {
