...
struct CFGVerification : public ModulePass {
  ...
  void addPrintf(BasicBlock &BB, IRBuilder<> &B, ArrayRef<Value *> Arguments) {
    // doPrintf
    auto *Context = &BB.getContext();
    auto CalleeF = BB.getModule()->getOrInsertFunction("printf",
                                                       FunctionType::get(IntegerType::getInt32Ty(*Context),
                                                                         PointerType::get(Type::getInt8Ty(*Context), 0),
                                                                         true /* this is var arg func type*/)
    );
    B.CreateCall(CalleeF, Arguments);
  }
   ...
   * @return Map of functions and their entry point functions
   */
  std::map<Function*, Function*> getFunctionsEntryPts(Module &Md) {
    std::map<Function*, Function*> Res;
    for (Function &Fn : Md) {
      if (!Fn.isDeclarationForLinker() && !Fn.hasExternalLinkage() && !Fn.hasNUses(0)) {
        Function *Entry = getEntryPt(Fn, Md, std::set<Function*>());
        if (Entry) {
          errs() << "Entry point found for function " << Fn.getName() <<": " << Entry->getName() << "()\n";
  ...
  Function* getEntryPt(Function &Fn, Module &Md, std::set<Function*> Visited) {
    ...
    for (User *U : Fn.users()) {
      ...
      if (Visited.find(Caller) == Visited.end()) { // the caller has not been visited -> recursively find EntryPt in Caller
        Visited.insert(&Fn);
   ...
   * @return Map of basic blocks, one entry for every split BasicBlock.
   ...
   * function.
   */
  std::map<BasicBlock*, BasicBlock*> splitBBsAtCalls(Module &Md) {
    std::map<BasicBlock*, BasicBlock*> Res;
    for (Function &Fn : Md) {
      ...
      if (!Fn.isDeclarationForLinker() && !Fn.hasExternalLinkage() && !Fn.hasNUses(0)) {
        for (User *U : Fn.users()) {
          ...
          errs() << "Use found for " << Fn.getName() << " into " << Caller->getParent()->getParent()->getName() << ". Splitting...\n";
          Instruction *SplitInstr = Caller->getNextNonDebugInstruction();
          BasicBlock *ParentBB = Caller->getParent()->splitBasicBlockBefore(SplitInstr);
          BasicBlock *ChildBB = &(*Fn.getBasicBlockList().begin());
          Res.insert(std::pair<BasicBlock*, BasicBlock*>(ParentBB, ChildBB));
        }
      }
    }
    return Res;
   ...
   * @return an associative map containing Md's basic blocks associated with their signatures
   */
  std::map<BasicBlock *, int> initializeBlocksSignatures(Module &Md) {
    errs() << "---------- SIGNATURES ----------\n";
   ...
   * @param Md
   * @return A map that assigns at each function a pair containing values
   * returned by the alloca of G and D at the beginning of such function.
   */
  void initializeEntryFunctions(Module &Md, std::map<BasicBlock *, int> BBSigs, std::map<BasicBlock*, BasicBlock*> BBCalls, Value* G, Value *D) {
    auto *IntType = llvm::Type::getInt32Ty(Md.getContext());
    for (Function &Fn : Md) {
      if (!Fn.isDeclarationForLinker() &&
          (Fn.hasExternalLinkage() || Fn.hasNUses(0))) {
        errs() << "Initializing entry function " << Fn.getName() << "... ";
   ...
   */
  int getNeighborSig(BasicBlock &BB, std::map<BasicBlock*, int> BBSigs,
                     std::map<BasicBlock*, BasicBlock*> BBCalls) {
    int N = 2;
  ...
  bool hasNPredecessorsOrMore(BasicBlock &BB, int N, std::map<BasicBlock*, int> BBSigs,
                              std::map<BasicBlock*, BasicBlock*> BBCalls) {
    int Count = 0;
   ...
   */
  void sortBasicBlocks(std::map<BasicBlock *, int> BBSigs, std::map<int, BasicBlock *> NewBBs) {
    for (auto Pair : BBSigs) { // for each element in the original set of blocks
      ...
      if (NewBBs.find(BBSig) != NewBBs.end()) { // check if a new basic block has been created for the signature
        ...
        errs() << "Working on BB with sig " << BBSig << "\n";
        // re-insert the CFGVerificationBB into the function in the right position
        errs() << "Removing parent "
               << CFGVerificationBB->getParent()->getName() << "\n";
        CFGVerificationBB->removeFromParent();
        errs() << "Removed!\n";
        CFGVerificationBB->insertInto(Pair.first->getParent(), Pair.first);
  ...
  void createCFGVerificationBB (BasicBlock &BB,
    ...
    errs() << "Created CFGVerificationBB (parent: " << CFGVerificationBB->getParent()->getName() << ")\n";
  }
  ...
  bool runOnModule(Module &Md) override {
    ...
    for (Function &Fn : Md) {
      if (!Fn.isDeclarationForLinker()) {
        // a function is an entry function if it has external linkage (i.e. is not static) or it is never used.
        bool IsEntry = Fn.hasExternalLinkage() || Fn.hasNUses(0);
        ...
        for (auto &Elem : BBSigs) {
          ...
          if (BB->getParent() == &Fn && (!IsEntry || (IsEntry && BB != &Fn.front()))) {
            errs() << "Creating CFGVerificationBB for BB with signature "
                   << Elem.second << "...\n";
        ...
        auto CalleeF = ErrBB->getModule()->getOrInsertFunction(
            "cfg_err_detected", FunctionType::getVoidTy(Fn.getContext()));
        ErrB.CreateCall(CalleeF);
        ErrB.CreateBr(ErrBB);
      }
    }
